...
"""
GPU tracing hooks for layerwise profiling.

This module provides PyTorch hooks for detailed layer-by-layer profiling
using the platform-agnostic tracing utilities. Works on both NVIDIA (nvtx)
and AMD (roctx) platforms.
"""
...
from vllm.utils.tracing import range_pop, range_push
...
@contextmanager
def layerwise_tracing_marker_context(
    module_name, module_obj, in_tensor=None, kwargs=None
):
    """Context manager for GPU tracing markers that automatically pushes on enter
    and pops on exit. Works on both NVIDIA (nvtx) and AMD (roctx) platforms.

    Example:
        with layerwise_tracing_marker_context("Module:MyModule", module,
                                              in_tensor=args, kwargs=kwargs) as ctx:
...
class PytHooks:
    ...
    def module_fwd_hook(self, module_obj, in_tensor, out_tensor):
        """Callback function that ends the tracing marker.
    ...
    def module_fwd_pre_hook(self, module_obj, in_tensor, kwargs):
        """Creates a tracing marker with the module name in it.
